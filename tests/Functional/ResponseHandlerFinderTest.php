<?php

declare(strict_types=1);

namespace Medas\HttpRequestHandlerTest\Functional;

use Medas\HttpRequestHandler\ResponseHandlerFinder;
use Medas\HttpRequestHandler\ResponseHandlers\{FileHandler, HtmlHandler, JsonHandler};
use PHPUnit\Framework\TestCase;

class ResponseHandlerFinderTest extends TestCase
{
    public function testGetHandlers(): void
    {
        $handlers = service(ResponseHandlerFinder::class)->get();

        self::assertInstanceOf(JsonHandler::class, $handlers[0]);
        self::assertInstanceOf(HtmlHandler::class, $handlers[1]);
        self::assertInstanceOf(FileHandler::class, $handlers[2]);
    }
}
